...
export type CreatorForList = { key: string; label: string; logo_url?: string | null };

export async function fetchCreatorsList(): Promise<CreatorForList[]> {
  ...
  return [...byKey.entries()]
    ...
    .map(([key, { label, logo_url, sortOrder }]) => ({ key, label, logo_url, sortOrder }))
    .sort((a, b) => {
