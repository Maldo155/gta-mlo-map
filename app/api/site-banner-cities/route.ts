...
const TABLE = "site_banner_cities";
...
export async function GET() {
  const { data, error } = await getSupabaseAdmin()
    .from(TABLE)
    .select(
      "title, subtitle, enabled, font_family, title_font_size, subtitle_font_size, title_font_weight, letter_spacing, subtitle_color, title_font_color, background_color, border_color, animation"
    )
    .eq("id", 1)
    .maybeSingle();
  ...
  if (!data) {
    return NextResponse.json({ title: null, subtitle: null, enabled: false });
  }
...
export async function PATCH(req: Request) {
  ...
  const title = body.title !== undefined ? (typeof body.title === "string" ? body.title.trim() || null : null) : undefined;
  const subtitle = body.subtitle !== undefined ? (typeof body.subtitle === "string" ? body.subtitle.trim() || null : null) : undefined;
